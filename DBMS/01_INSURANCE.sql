...
SELECT COUNT(DISTINCT P.DID) FROM ACCIDENT A,PARTICIPATED P WHERE A.REP_NUM=P.REP_NUM AND A.ACC_DATE LIKE '2008%';

/*Find the number of accidents in which cars belonging to a specific model (example 'Lancer')  were involved.*/
SELECT COUNT(DISTINCT P.REP_NUM) FROM CAR C,PARTICIPATED P WHERE C.REG_NUM=P.REG_NUM AND C.MODEL='LANCER';
...
DELETE FROM PARTICIPATED WHERE D_AMT<(SELECT AVERAGE FROM (SELECT AVG(D_AMT) AS AVERAGE FROM PARTICIPATED) AS TEMP);
SELECT * FROM PARTICIPATED;

/*LIST THE NAME OF DRIVERS WHOSE DAMAGE IS GREATER THAN THE AVERAGE DAMAGE AMOUNT.*/
SELECT D.NAME FROM PARTICIPATED P,DRIVER D WHERE D.DID=P.DID AND P.D_AMT>(SELECT AVG(D_AMT) FROM PARTICIPATED);
