...
INSERT INTO CATALOG VALUES(30,400,150);
INSERT INTO CATALOG VALUES(30,100,100);
...
PARTS P WHERE P.PID NOT IN (SELECT C.PID FROM CATALOG C WHERE C.SID=S.SID));
...
SELECT P.PNAME FROM PARTS P,SUPPLIERS S,CATALOG C WHERE P.PID=C.PID AND 
C.SID=S.SID AND P.PNAME NOT IN (SELECT DISTINCT P.PNAME FROM PARTS P,CATALOG C,
  SUPPLIERS S WHERE P.PID=C.PID AND C.SID=S.SID AND S.SNAME!='ACMEWIDGET');
...
SELECT C1.SID,C1.PID,C1.COST FROM CATALOG C1 WHERE C1.COST>(SELECT AVG(C.COST)
           FROM CATALOG C WHERE C.PID=C1.PID GROUP BY C.PID);

/*For each part, find the sname of the supplier who charges the most for that part.*/
SELECT S.SNAME,C.PID FROM CATALOG C,SUPPLIERS S,PARTS P WHERE S.SID=C.SID
AND P.PID=C.PID AND C.COST=(SELECT MAX(C1.COST) FROM CATALOG C1 WHERE
     C1.PID=C.PID GROUP BY C1.PID);

/*Find the sids of suppliers who supply only red parts.*/
/*NOO*/
SELECT C.SID FROM CATALOG C WHERE NOT EXISTS ( SELECT * FROM PARTS
               P WHERE P.PID = C.PID AND P.COLOR!='RED');

/*Find the sids of suppliers who supply a red part and a green part.*/
SELECT DISTINCT C1.SID FROM CATALOG C1 WHERE C1.SID=ANY(SELECT DISTINCT
  C.SID FROM CATALOG C,PARTS P WHERE P.PID=C.PID AND P.COLOR='RED' AND 
 C.SID=ANY(SELECT DISTINCT C.SID FROM CATALOG C,PARTS P WHERE P.PID=C.PID AND P.COLOR='GREEN') 
                                     AND C.SID=C1.SID);

/*Find the sids of suppliers who supply a red part or a green part.*/
SELECT DISTINCT SID FROM CATALOG WHERE SID IN(SELECT DISTINCT C.SID FROM
  CATALOG C,PARTS P WHERE P.PID=C.PID AND P.COLOR='RED') OR SID IN(SELECT DISTINCT
        C.SID FROM CATALOG C,PARTS P WHERE P.PID=C.PID AND P.COLOR='GREEN');
...
/*NOO*/
SELECT C.SID FROM CATALOG C WHERE NOT EXISTS (SELECT * FROM PARTS P
...
SELECT C2.SID,MAX(C2.COST) FROM CATALOG C2 WHERE C2.SID IN (SELECT 
  DISTINCT C1.SID FROM CATALOG C1 WHERE C1.SID=ANY(SELECT DISTINCT C.SID FROM
    CATALOG C,PARTS P WHERE P.PID=C.PID AND P.COLOR='RED' AND C.SID=ANY(SELECT DISTINCT
      C.SID FROM CATALOG C,PARTS P WHERE P.PID=C.PID AND P.COLOR='GREEN') AND 
                     C.SID=C1.SID)) GROUP BY C2.SID;
