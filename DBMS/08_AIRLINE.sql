...
SELECT DISTINCT A.A_NAME FROM AIRCRAFT A,EMPLOYEE E,CERTIFIED C
...
E.SALARY<(SELECT MIN(PRICE) FROM FLIGHTS WHERE TO_PLACE="FRANKFURT" AND FROM_PLACE="BENGALURU");
...
SELECT A.A_NAME,AVG(E.SALARY) FROM AIRCRAFT A,CERTIFIED C,EMPLOYEE E WHERE
C.E_ID=E.E_ID AND C.A_ID=A.A_ID AND A.A_ID IN(SELECT A_ID FROM AIRCRAFT WHERE
CRUISING_RANGE>1000) GROUP BY A.A_ID;

/*Find the names of pilots certified for some Boeing aircraft*/
SELECT DISTINCT E.E_NAME FROM EMPLOYEE E,AIRCRAFT A,CERTIFIED C WHERE
E.E_ID=C.E_ID AND A.A_ID=C.A_ID AND A.A_ID IN (SELECT A_ID FROM AIRCRAFT
WHERE A_NAME LIKE '%AIR%');

/*Find the aids of all aircraft that can be used on routes from Bangalore to Frankfurt.*/
SELECT A_ID FROM AIRCRAFT WHERE CRUISING_RANGE>ANY(SELECT DISTANCE FROM
FLIGHTS WHERE FROM_PLACE="BENGALURU" AND TO_PLACE="FRANKFURT");

/*Print the name and salary of every non-pilot whose salary is more than the average salary for pilots*/
SELECT E_NAME,SALARY FROM EMPLOYEE WHERE SALARY>(SELECT AVG(E.SALARY)
FROM EMPLOYEE E,CERTIFIED C WHERE E.E_ID=C.E_ID) AND E_ID NOT IN(SELECT E_ID FROM CERTIFIED);

/*Identify the routes that can be piloted by every pilot who makes more than 100,000*/
SELECT DISTINCT F.FROM_PLACE,F.TO_PLACE FROM FLIGHTS F,AIRCRAFT A,CERTIFIED C
WHERE C.A_ID=A.A_ID AND A.CRUISING_RANGE>F.DISTANCE AND C.E_ID IN
(SELECT E_ID FROM EMPLOYEE WHERE SALARY>50000);
...
SELECT DISTINCT E.E_NAME FROM EMPLOYEE E,CERTIFIED C WHERE E.E_ID=C.E_ID
AND C.A_ID IN (SELECT A_ID FROM AIRCRAFT WHERE CRUISING_RANGE>3000) AND
C.A_ID NOT IN(SELECT A_ID FROM AIRCRAFT WHERE A_NAME LIKE '%AIR%');

/*Compute the difference between the average salary of a pilot and the average salary of all employees (including pilots)*/
SELECT T2.AVERAGE-T1.AVERAGE FROM (SELECT AVG(SALARY)AS AVERAGE FROM EMPLOYEE)
AS T1,(SELECT AVG(E.SALARY) AS AVERAGE FROM EMPLOYEE E,CERTIFIED C WHERE C.E_ID=E.E_ID) AS T2;
