...
SELECT S.SNAME FROM STUDENT S,FACULTY F,CLASS C,ENROLLED E WHERE S.SNUM=E.SNUM AND E.CNAME=C.CNAME AND C.FID=F.FID AND F.FNAME="PROF. MURTHY" AND S.LVL="JR";

/*Find the names of all classes that either meet in room 303 or have five or more Students enrolled.*/
(SELECT CNAME FROM CLASS WHERE ROOM=303) UNION (SELECT CNAME FROM ENROLLED GROUP BY CNAME HAVING COUNT(SNUM)>=3);

/*Find the names of all students who are enrolled in two classes that meet at the same time.*/
/*NOT DONE*/
SELECT DISTINCT S.SNAME FROM STUDENT S WHERE S.SNUM IN (SELECT DISTINCT E1.SNUM FROM ENROLLED E1,ENROLLED E2,CLASS C1,CLASS C2 WHERE E1.SNUM=E2.SNUM AND E1.CNAME!=E2.CNAME AND E1.CNAME=C1.CNAME AND E2.CNAME=C2.CNAME AND C1.MEETS_AT=C2.MEETS_AT); 
...
SELECT DISTINCT F.FNAME FROM FACULTY F WHERE (SELECT COUNT(E.SNUM) FROM ENROLLED E,CLASS C WHERE E.CNAME=C.CNAME AND C.FID=F.FID)>5;
...
SELECT DISTINCT SNAME FROM STUDENT WHERE SNUM IN(SELECT E.SNUM FROM ENROLLED E GROUP BY E.SNUM HAVING COUNT(E.CNAME)>=ALL(SELECT COUNT(E1.CNAME) FROM ENROLLED E1 GROUP BY E1.SNUM));
