CREATE DATABASE STUDENT;
USE STUDENT;

CREATE TABLE STUDENT(REGNO INT,SNAME VARCHAR(10),MAJOR VARCHAR(10),BDATE DATE,PRIMARY KEY(REGNO));

CREATE TABLE COURSE(CID INT,CNAME VARCHAR(10),DEPT VARCHAR(10),PRIMARY KEY(CID));

CREATE TABLE BOOK(BID INT,BNAME VARCHAR(10),PUBLISHER VARCHAR(10),AUTHOR VARCHAR(10),PRIMARY KEY(BID));

CREATE TABLE BOOK_ADOPTION(CID INT,SEM INT,BID INT,PRIMARY KEY(CID,BID),FOREIGN KEY(CID) REFERENCES COURSE(CID),FOREIGN KEY(BID) REFERENCES BOOK(BID));

CREATE TABLE ENROLL(REGNO INT,CID INT,SEM INT,MARKS INT,PRIMARY KEY(REGNO,CID),FOREIGN KEY(REGNO) REFERENCES STUDENT(REGNO),FOREIGN KEY(CID) REFERENCES COURSE(CID));

INSERT INTO STUDENT VALUES(1,'RAJATH','ISE','01-02-02');
INSERT INTO STUDENT VALUES(2,'SAM','CSE','12-10-00');
INSERT INTO STUDENT VALUES(3,'SHAAN','MECH','21-02-01');
INSERT INTO STUDENT VALUES(4,'TANYA','EC','23-09-01');
INSERT INTO STUDENT VALUES(5,'AYMAN','CSE','07-10-00');
SELECT *FROM STUDENT;

INSERT INTO COURSE VALUES(11,'DCS','CSE');
INSERT INTO COURSE VALUES(22,'ADA','CSE');
INSERT INTO COURSE VALUES(33,'CN','EC');
INSERT INTO COURSE VALUES(44,'TD','MECH');
INSERT INTO COURSE VALUES(55,'MUP','EC');
SELECT * FROM COURSE;

INSERT INTO BOOK VALUES(772,'VB','PHI','HOLZNER');
INSERT INTO BOOK VALUES(114,'DS WITH C','SAPNA','NANDAGOPAL');
INSERT INTO BOOK VALUES(440,'C PROG','PHI','BALAGURU');
INSERT INTO BOOK VALUES(556,'CN','PHI','TENENBAUM');
INSERT INTO BOOK VALUES(338,'MP','TATA','BREY');
INSERT INTO BOOK VALUES(331,'MC','TATA','BDBD');
SELECT * FROM BOOK;

INSERT INTO BOOK_ADOPTION VALUES(11,3,772);
INSERT INTO BOOK_ADOPTION VALUES(22,4,772);
INSERT INTO BOOK_ADOPTION VALUES(11,5,440);
INSERT INTO BOOK_ADOPTION VALUES(11,8,556);
INSERT INTO BOOK_ADOPTION VALUES(55,4,338);
INSERT INTO BOOK_ADOPTION VALUES(55,5,331);
INSERT INTO BOOK_ADOPTION VALUES(33,3,331);
SELECT * FROM BOOK_ADOPTION;

INSERT INTO ENROLL VALUES(1,22,5,76);
INSERT INTO ENROLL VALUES(1,11,3,90);
INSERT INTO ENROLL VALUES(2,33,6,55);
INSERT INTO ENROLL VALUES(3,22,5,75);
INSERT INTO ENROLL VALUES(5,44,5,75);
SELECT * FROM ENROLL;

/*iii. Demonstrate how you add a new text book to the database and make this book be adopted by some department. */
INSERT INTO BOOK VALUES(650,'ALGORITHM','SAPNA','REEMA');
INSERT INTO BOOK_ADOPTION VALUES(22,4,650);
SELECT * FROM BOOK;
SELECT * FROM BOOK_ADOPTION;

/*iv. Produce a list of text books (include Course #, Book-ISBN, Book-title) in the alphabetical order for courses offered by the ‘CS’ department that use more than two books. */
SELECT C.CNAME,B.BID,B.BNAME FROM BOOK B,COURSE C,BOOK_ADOPTION BA WHERE C.CID=BA.CID AND B.BID=BA.BID AND C.CID IN (SELECT BA.CID FROM COURSE C,BOOK_ADOPTION BA WHERE C.CID=BA.CID AND C.DEPT="CSE" GROUP BY BA.CID HAVING COUNT(*)>2);

/*v. List any department that has all its adopted books published by a specific publisher*/
SELECT DISTINCT DEPT FROM COURSE WHERE DEPT IN (SELECT C.DEPT FROM COURSE C,BOOK B,BOOK_ADOPTION BA WHERE BA.CID=C.CID AND B.BID=BA.BID AND B.PUBLISHER="TATA") AND DEPT NOT IN(SELECT C.DEPT FROM COURSE C,BOOK B,BOOK_ADOPTION BA WHERE BA.CID=C.CID AND B.BID=BA.BID AND B.PUBLISHER!="TATA");