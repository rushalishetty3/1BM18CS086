...
SELECT DISTINCT D.CUSTOMER_NAME FROM DEPOSITOR D,BRANCH B,ACCOUNTS A
WHERE A.ACC_NO=D.ACC_NO AND B.BRANCH_NAME=A.BRANCH_NAME AND B.BRANCH_NAME="FASER TOWN"
AND ( SELECT A.ACC_NO FROM ACCOUNTS A HAVING COUNT(*)>=2);
...
CREATE VIEW SUM_AMT(BRANCH_NAME,AMOUNT) AS SELECT BRANCH_NAME,SUM(AMOUNT)
